...
const borshString = (s) =>
  Buffer.concat([u32le(Buffer.byteLength(s)), Buffer.from(s, 'utf8')]);
const borshU128 = (x) => {
...
const cases = [
  ...
  {
    prefix: [0x00, 0x07],
    account: 'verylongaccountname-with-dashes_and.subaccounts.near',
...
for (const t of cases) {
  ...
  console.log(
    `${ok ? 'PASS' : 'FAIL'}  prefix=[${t.prefix}]  acct=${t.account}  bal=${
      t.balance
    }`,
  );
...
const nonU128 = {
  ...
  key_base64: Buffer.concat([
    Buffer.from([0x53]),
    borshString('alice.near'),
  ]).toString('base64'),
  value_base64: Buffer.alloc(24).toString('base64'),
};
const hashedKey = {
  account_id: 'token.near',
  key_base64: Buffer.concat([
    Buffer.from([0x61]),
    createHash('sha256').update('alice.near').digest(),
  ]).toString('base64'),
  value_base64: borshU128('5').toString('base64'),
};
const g1 = decode(nonU128) === null;
const g2 = decode(hashedKey) === null;
console.log(`${g1 ? 'PASS' : 'FAIL'}  guard: 24-byte (non-u128) value skipped`);
console.log(
  `${g2 ? 'PASS' : 'FAIL'}  guard: hashed/opaque key skipped -> RPC fallback`,
);

const allOk = pass === cases.length && g1 && g2;
console.log(
  `\n${pass}/${cases.length} positive cases + ${
    (g1 ? 1 : 0) + (g2 ? 1 : 0)
  }/2 guards`,
);
