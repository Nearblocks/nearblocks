...
export const RpcMenu = ({ setShowModal }: any) => {
  const { getCustomRpc, network, providers, removeRpc } = useNetworkStore();
  const customRpc = getCustomRpc();
  const rpcUrl = useRpcStore((state) => state.rpc);
  const setRpc = useRpcStore((state) => state.setRpc);
  const [allProviders, setAllProviders] = useState<Rpc[]>([
    { name: '', url: '' },
  ]);

  useEffect(() => {
    setAllProviders([...providers, ...customRpc]);
  }, [network, customRpc, providers]);
...
export const RpcModal: React.FC<RpcModalProps> = ({
  ...
  const { addRpc, getCustomRpc, providers } = useNetworkStore();
  const customRpc = getCustomRpc();
