...
import { EventContract, FTEventEntry, FTMetaArgsRefToken } from '#types/types';

const EVENT_TYPE = EventType.TOKEN_REF_FINANCE_NEAR;
...
const matchActions = (
  ...
  switch (action.FunctionCall.methodName) {
    case 'new': {
      ...
      if (!args.owner || !BigInt(args.total_supply)) {
        return [];
      }
