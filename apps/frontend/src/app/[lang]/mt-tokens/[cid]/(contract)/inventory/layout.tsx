...
type Props = LayoutProps<'/[lang]/mt-tokens/[cid]/inventory'>;

const InventoryLayout = async ({ children, params }: Props) => {
    ...
    <Card>
      <CardContent className="text-body-sm p-3">
        <ScrollArea className="mb-3 w-full whitespace-nowrap">
          <TabLinks>
            <TabLink asChild>
              <ActiveLink exact href={`/mt-tokens/${cid}/inventory`}>
                {t('inventory.ftTab')}
              </ActiveLink>
            </TabLink>
            <TabLink asChild>
              <ActiveLink href={`/mt-tokens/${cid}/inventory/nfts`}>
                {t('inventory.nftTab')}
              </ActiveLink>
            </TabLink>
          </TabLinks>
