...
export const IntentsSwapsChart = ({ loading, statsPromise }: Props) => {
  ...
  return (
    <Card>
      <ChartHeader
        description={t('intentsSwaps.description')}
        logView={logView}
              ...
              isEmpty ? (
                ...
                <AnalyticsChart height={560}>
                  ...
                  <YAxis
                    ...
                    labels={yAxisLabel}
                    opposite={false}
                    title={{ text: t('intentsSwaps.yAxis') }}
                  ...
                  <Column.Series
                    ...
                    options={{
                      id: 'daily',
                      name: t('intentsSwaps.seriesDaily'),
                      yAxis: 0,
                    }}
                  />
                  <YAxis
                    className="stroke-0"
                    labels={yAxisLabel}
                    title={{ text: t('intentsSwaps.yAxisCumulative') }}
                  />
                  <Line.Series
                    data={cumulative}
                    options={{
                      id: 'cumulative',
                      name: t('intentsSwaps.seriesCumulative'),
                      yAxis: 1,
                    }}
                  />
...
export const IntentsSwapsChartMini = ({ loading, statsPromise }: Props) => {
  const { t } = useLocale('charts');
  const stats = !loading && statsPromise ? use(statsPromise) : null;
        ...
        {() =>
          isEmpty ? (
            ...
            <MiniChart height={220}>
              <Area.Series
                data={daily}
                options={{ name: t('intentsSwaps.seriesDaily') }}
              />
              <Tooltip formatter={tooltipFormatter} shared />
            </MiniChart>
          )
