...
import { DailyTxnStats } from 'nb-schemas';
...
type Props = {
  loading?: boolean;
  statsPromise?: Promise<DailyTxnStats[] | null>;
};

const getData = (stats: DailyTxnStats[] | null) =>
  (stats ?? [])
    .toReversed()
    .filter((item) => item.receipts !== null)
    .map(
      (item) =>
        [new Date(item.date).getTime(), +item.receipts!] as [number, number],
    );
...
export const ReceiptsChart = ({ loading, statsPromise }: Props) => {
              ...
              isEmpty ? (
                ...
                <AnalyticsChart height={560}>
                  ...
                  <YAxis
                    ...
                    labels={yAxisLabel}
                    opposite={false}
                    title={{ text: t('receipts.yAxis') }}
                    type={logView ? 'logarithmic' : 'linear'}
                  />
                  <Area.Series
                    data={data}
                    options={{ name: t('receipts.series') }}
                  />
...
export const ReceiptsChartMini = ({ loading, statsPromise }: Props) => {
  const { t } = useLocale('charts');
  const stats = !loading && statsPromise ? use(statsPromise) : null;
