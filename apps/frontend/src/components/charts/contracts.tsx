...
import { AddressStats } from 'nb-schemas';
...
type Props = {
  loading?: boolean;
  statsPromise?: Promise<AddressStats[] | null>;
};

const getData = (stats: AddressStats[] | null) =>
  (stats ?? [])
    .toReversed()
    .filter((item) => item.active_contracts !== null)
    .map(
      (item) =>
        [new Date(item.date).getTime(), +item.active_contracts!] as [
          number,
          number,
        ],
    );
...
export const ContractsChart = ({ loading, statsPromise }: Props) => {
              ...
              isEmpty ? (
                ...
                <AnalyticsChart height={560}>
                  ...
                  <YAxis
                    ...
                    labels={yAxisLabel}
                    opposite={false}
                    title={{ text: t('contracts.yAxis') }}
                    type={logView ? 'logarithmic' : 'linear'}
                  />
                  <Area.Series
                    data={data}
                    options={{ name: t('contracts.series') }}
                  />
...
export const ContractsChartMini = ({ loading, statsPromise }: Props) => {
  const { t } = useLocale('charts');
  const stats = !loading && statsPromise ? use(statsPromise) : null;
