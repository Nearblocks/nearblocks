...
export const TxnsChart = memo(({ data, height = 420 }: Props) => {
  ...
  return (
    <Chart chartConstructor="stockChart" options={chartOptions}>
      <Line.Series
        data={seriesData.transactions}
        options={{ name: 'Transactions' }}
      />
      <Line.Series
        data={seriesData.uniqueOutgoing}
        options={{ name: 'Unique Outgoing Address' }}
      />
      <Line.Series
        data={seriesData.uniqueIncoming}
        options={{ name: 'Unique Incoming Address' }}
      />
    </Chart>
  );
});

TxnsChart.displayName = 'TxnsChart';
