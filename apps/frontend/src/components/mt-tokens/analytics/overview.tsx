...
const Heatmap = dynamic(
  () => import('@/components/address/analytics/heatmap').then((m) => m.Heatmap),
  { loading: () => <Skeleton className="h-46 w-full" />, ssr: false },
);

type Props = {
  heatmapPromise?: Promise<MTContractStatsHeatmapRes>;
  loading?: boolean;
  overviewPromise?: Promise<MTContractStatsOverviewRes>;
};
...
export const OverviewChart = ({
  ...
  const overviewRes =
    !loading && overviewPromise ? use(overviewPromise) : undefined;
  const heatmapRes =
    !loading && heatmapPromise ? use(heatmapPromise) : undefined;

  const overview: MTContractStatsOverview | null = overviewRes?.data ?? null;
  const heatmapData: MTContractStatsHeatmap[] = heatmapRes?.data ?? [];

  const mappedHeatmap = heatmapData.map((d) => ({
    date: d.date,
    txns: d.transfers_count,
  }));
