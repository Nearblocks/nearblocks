...
const APIKeys = ({
  ...
  const { data, loading, error, mutate } = useAuth('/keys');
      ...
      <UserLayout title="API Keys">
        ...
        <div className="w-full bg-white dark:bg-black-600 dark:text-neargray-10 rounded-xl soft-shadow h-fit mb-4 mt-8">
                ...
                <tbody className="bg-white dark:bg-black-600 divide-y divide-gray-200">
                  {(loading || error) &&
                    [...Array(5)].map((_, i) => (
                      <tr key={i} className="hover:bg-blue-900/5 h-[53px]">
                        <td className="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-neargray-10 uppercase tracking-wider">
                  ...
                  {!loading && keys?.length === 0 && (
                    <tr className="h-[53px]">
                      <td colSpan={100} className="text-gray-400 text-xs">
                        <div className="w-full bg-white dark:bg-black-200 soft-shadow h-fit">
                  ...
                  {keys?.map((key: ApiKey) => (
                    <tr key={key.id} className="hover:bg-blue-900/5">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-primary align-top">
                        <div className="flex">
                          <p className="mr-2 text-black dark:text-neargray-10">
                            {key?.token}
                          </p>
                          <button
                            type="button"
                      ...
                      <td className="px-6 py-4 whitespace-nowrap flex  items-center text-xs text-gray-500 dark:text-neargray-10 align-top">
                        <div
                          className="flex items-center border border-green-500 dark:border-green-250 rounded-md px-2 py-1 hover:bg-primary-100 dark:hover:bg-black-200"
                          onClick={() => onToggleEdit(key.id)}
                        >
                          <Edit className="text-primary dark:text-neargray-10" />{' '}
                          <p className="ml-1 text-green-500 dark:text-neargray-10 cursor-pointer">
                            Edit
                          </p>
                        </div>
                        <div className="flex items-center border border-green-500 dark:border-green-250 rounded-md px-2 py-1 hover:bg-primary-100 dark:hover:bg-black-200 ml-3">
                          <Link
                            passHref
          ...
          <div className="flex items-center justify-end px-6 py-4">
            <button
              onClick={onToggleAdd}
              className="text-[13px] text-white font-semibold px-6 py-2 bg-green-500 dark:bg-green-250 hover:-translate-y-1 hover:scale-100 duration-300 hover:shadow-md hover:shadow-primary rounded"
            >
              Add key
            </button>
          </div>
