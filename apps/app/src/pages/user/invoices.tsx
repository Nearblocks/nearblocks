        ...
        <div className="w-full bg-white dark:bg-black-600 dark:text-neargray-10 rounded-xl soft-shadow h-fit mb-4">
          <div className="px-5 py-5 flex justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400 dark:text-neargray-10 mt-2">
                List all invoices
              </p>
            </div>
            <Pagination
          ...
          <div className="flex border-b dark:border-black-200">
            <div className="overflow-x-auto w-full">
              <table className="w-full divide-y border-t dark:border-black-200 dark:divide-black-200">
                <thead className="bg-gray-100 dark:bg-black-600">
                  <tr>
                    <th
                      scope="col"
                      className="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-neargray-10 uppercase tracking-wider whitespace-nowrap"
                    >
                      Invoice Number
                    </th>
                    <th
                      scope="col"
                      className="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-neargray-10 uppercase tracking-wider"
                    >
                      Amount
                    </th>
                    <th
                ...
                <tbody className="bg-white dark:bg-black-600 divide-y divide-gray-200">
                  {loading &&
                    [...Array(5)].map((_, i) => (
                      <tr key={i} className="hover:bg-blue-900/5 h-[53px]">
                        <td className="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-neargray-50 uppercase tracking-wider">
                          <Skeleton className="h-4" />
                        </td>
                        <td className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                          <Skeleton className="h-4" />
                        </td>
                        <td className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                          <Skeleton className="h-4" />
                  ...
                  {!loading && (!invoices || invoices?.length < 1) && (
                    <tr className="h-[53px]">
                      <td
                        colSpan={100}
                        className="text-gray-400 dark:text-neargray-10 text-xs"
                      >
                  ...
                  {invoices?.map((invoice: Invoice) => (
                    <tr key={invoice.id} className="hover:bg-blue-900/5">
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-black dark:bg-black-200 dark:text-neargray-10 align-top">
                        {invoice.number}
                        {role === 'publisher' && (
                          <Tooltip
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs dark:bg-black-200 dark:text-neargray-10  text-primary align-top">
                        <div className="flex">
                          <p className="mr-2 text-black dark:bg-black-600 dark:text-neargray-10">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-black dark:bg-black-200 dark:text-neargray-10 align-top">
                        {dayjs.unix(invoice?.period_end).format('MMMM D, YYYY')}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-black dark:bg-black-200 dark:text-neargray-10 align-top ml-3">
                        {invoice?.status}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-black dark:bg-black-200 dark:text-neargray-10 align-top">
                        <a href={invoice.invoice_pdf}>
                          <div className="flex items-center border border-green-500  dark:border-green-250 rounded-md px-2 hover:bg-primary-100">
                            <FaDownload className="text-green-500 dark:text-neargray-10 " />{' '}
                            <p className="ml-1 text-green-500 dark:text-neargray-10 cursor-pointer">
                              Download
                            </p>
                          </div>
                        </a>
