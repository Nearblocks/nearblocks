...
const VerifierActions: React.FC<ContractFormProps> = ({
  accountId,
  network,
    ...
    const fetchData = async () => {
      try {
        ...
        if (!contractOnChainHash) {
          const onChainResponse = await contractCode(accountId);
  ...
  const Loader = (props: { wrapperClassName?: string }) => {
    return (
      <div
        className={`bg-gray-200 dark:bg-black-200 h-10 rounded shadow-sm animate-pulse ${props.wrapperClassName}`}
      ></div>
    );
  };
        ...
        {contractError ? (
          ...
          >
            ...
            <p className="inline-block text-sm mt-2">
              ...
              exist, may not be deployed, or there might be an issue fetching
              the contract data. Please verify the contract address and try
              again. If you are the contract owner and need assistance, check
              out the&nbsp;
              <a
          ...
          >
            ...
            <p className="inline-block text-sm mt-2">
              ...
              NEP-330 standard, which is required for contract source metadata
              and verification. To make your contract verifiable, you need to
              implement the NEP-330 standard and include contract source
              metadata. For detailed instructions on how to add NEP-330 support
              to your contract, check out the
              <a
                      ...
                      <>
                        ...
                        {!hasNep330 && (
                          <p className="text-red-500 text-xs mt-1">
                            Contract needs to be NEP-330 standard for
                            verification
                          </p>
                        )}
                      </>
