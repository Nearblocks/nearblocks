...
interface Props {
  ...
  priceDataPromise: Promise<any>;
}

export default function Details(props: Props) {
  const data = use(props?.hashDataPromise);
  const price = use(props?.priceDataPromise);
  ...
  const block =
    !data || !data?.data || data?.errors ? blockInfo?.data : data?.data;
