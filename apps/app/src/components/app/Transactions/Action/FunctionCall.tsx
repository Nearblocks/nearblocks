...
const FunctionCall = (action: any) => {
  const params = useParams();

  return (
    <div className="action flex flex-wrap items-center break-">
      {action?.receiptId && params?.hash ? (
        <Link href={`/txns/${params?.hash}?tab=execution#${action?.receiptId}`}>
      ...
      <span className="font-bold px-1">
        ...
        <span className="font-normal pl-1">
          <Tooltip
            ...
            position="bottom"
            tooltip={action.args.method_name}
          >
            <span className="bg-blue-900/10 text-xs text-nearblue-600 dark:text-neargray-10 rounded-xl px-2 py-0.5 max-w-[240px] inline-flex truncate">
              <span className="block truncate">{action.args.method_name}</span>
            </span>
          </Tooltip>
        </span>
      </span>
