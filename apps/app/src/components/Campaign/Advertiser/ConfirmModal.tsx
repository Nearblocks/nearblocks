...
const ConfirmModal = ({
  ...
  return (
    <>
      <DialogOverlay className="fixed bg-gray-600 bg-opacity-20 inset-0 z-30 flex items-start mt-32 justify-center">
        <DialogContent className="w-full max-w-lg mx-auto bg-white dark:bg-black-600 dark:text-neargray-10 shadow-lg rounded-lg overflow-hidden px-4 md:px-0">
          ...
          <div className="px-6 pb-5 pt-2">
            <div className="py-2 pb-10">
              <p className="text-gray-400 dark:text-neargray-10 text-sm">
            ...
            <div className="flex items-center justify-end  py-4">
              <p
                onClick={() => setConfirmOpen(false)}
                className="text-[13px] delay-300 duration-300 mx-1 dark:text-neargray-10 hover:bg-gray-200 dark:hover:bg-black-200 px-4 py-2 rounded cursor-pointer"
              >
                Cancel
              </p>
              <button
                onClick={onSubmit}
                className={`text-sm text-[13px] px-4 focus:outline-none text-white text-center font-semibold py-2 bg-green-500 dark:bg-green-250 rounded transition ease-in-out delay-150 ${
                  buttonLoading
                    ? 'opacity-50 cursor-not-allowed'
                    : 'hover:-translate-y-1 hover:scale-100 hover:shadow-md hover:shadow-primary'
                }`}
