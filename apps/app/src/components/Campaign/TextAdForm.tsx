...
import { textCampaignValidation } from '@/utils/validationSchema';
...
const TextAdForm = ({
  ...
  const onSubmit = async (values: FormikValues) => {
    ...
    try {
      ...
      // if (!toast.isActive('campaign-update-error')) {
      toast.error(message, {
  ...
  return (
    <>
      <ToastContainer />
      <div className="rounded-xl border dark:border-black-200 soft-shadow">
        <div className="px-5 py-5 border-b dark:border-black-200">
          <p className="text-base text-black dark:text-neargray-10">
            Campaign Information
          </p>
        </div>
        <p className="text-sm text-gray-400 dark:text-neargray-10 mt-2 px-6 py-4">
        ...
        <div>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-start col-span-2">
                <input
                  ...
                  className="w-full border px-3 py-2 dark:text-neargray-10 text-sm outline-primary rounded-md dark:bg-black-200  dark:border-neargray-100"
                />
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-start col-span-2">
                <input
                  ...
                  disabled={isSubscriptionCancelled}
                  type="text"
                  className="text-black dark:text-neargray-10 border dark:bg-black-200 dark:border-neargray-100 px-3 py-2 text-sm outline-primary rounded-md w-full mt-2"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-1 items-center px-6 py-4  border-b dark:border-black-200">
            <div className="text-gray-500 dark:text-neargray-10 text-sm flex flex-col items-start">
              <div className="flex items-center">
                <Avatar />{' '}
                <span className="ml-2">Upload Icon (20px - 20px)</span>
              </div>
              <p className="text-gray-500 text-xs mt-2 italic ml-4">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-start col-span-2">
                <div className="h-[70px] border w-full relative text-black dark:text-neargray-10 dark:bg-black-200 dark:border-neargray-100 px-3 py-2 text-sm outline-primary rounded-md">
                  <label className="w-full h-full flex items-center justify-center  hover:cursor-pointer overflow-hidden">
                    ...
                    {iconPreview?.icon || campaignData?.data?.icon ? (
                      ...
                      <span className="text-sm text-gray-400 dark:text-neargray-10">
                        Select Icon
                      </span>
                    )}
                  </label>
          ...
          <div className="ml-[51px] mr-6 py-4 flex flex-row justify-end">
            {campaignData &&
              ...
              campaignData?.data?.icon && (
                <button
                  ...
                  className="mx-2 text-sm bg-green-500 dark:bg-green-250 text-[13px] px-10  focus:outline-none text-white text-center font-semibold py-2 rounded  hover:-translate-y-1 hover:scale-100 duration-300 hover:shadow-md hover:shadow-primary"
                >
                  Preview
                </button>
              )}
            ...
            <button
              ...
              className={`text-sm text-[13px] px-10
               focus:outline-none text-white text-center font-semibold py-2 rounded ${
                 isSubmitting || isSubscriptionCancelled
                   ? 'cursor-not-allowed bg-primary-100'
                   : ' bg-green-500 dark:bg-green-250 hover:-translate-y-1 hover:scale-100 duration-300 hover:shadow-md hover:shadow-primary'
               } `}
            >
              Update
            </button>
