...
const Burn = ({ event, data }: Props) => {
  ...
  return (
    <div className=" flex flex-col break-all leading-7 !justify-between">
      {data.map((item) => {
        ...
        if (token_ids && amounts && token_ids.length === amounts.length) {
          return token_ids.map((token, idx) => (
            <div
              key={idx}
              className={`flex flex-wrap items-start gap-1 sm:!gap-1 ${
            ...
            >
              ...
              <span className="font-bold text-sm sm:text-xs pt-1">
                Withdraw
              </span>
