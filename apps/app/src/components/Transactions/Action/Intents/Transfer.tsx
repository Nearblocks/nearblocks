      ...
      <div className="flex flex-wrap items-center text-sm sm:text-base mt-1">
        {event?.receiptId && hash ? (
          <Link href={`/txns/${hash}#execution#${event?.receiptId}`}>
            <FaRight className="text-gray-400 text-sm sm:text-base cursor-pointer" />
          </Link>
        ) : (
          <FaRight className="text-gray-400 text-sm sm:text-base" />
        )}
        <span className="font-bold pl-1 text-sm sm:text-base">Swap</span>
          ...
          tokens?.map(({ token, amount }, index) => {
            ...
            return (
              <div key={index} className="flex items-center">
                ...
                {isLastToken && (
                  <>
                    <span className="font-bold text-gray pl-1 text-sm sm:text-base">
                      For
                    </span>
                    <TokenInfo
                      contract={contractName}
