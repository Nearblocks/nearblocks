...
const Transfers = () => {
  ...
  const { data: totalCount, loading: countLoading } =
    useFetch('fts/txns/count');
  const { data: syncStatus, loading: syncLoading } = useFetch('sync/status');
  const tokens = data?.txns;
  const cursor = data?.cursor;
  const count = totalCount?.txns?.[0]?.count || 0;
  const status = syncStatus?.status?.indexers?.events || {
    height: 0,
    sync: true,
  };
