...
export function truncateString(str: string, maxLength: number, suffix: string) {
  ...
  return str && str.substring(0, maxLength) + suffix;
}
...
export function fiatValue(big: string, price: string) {
  ...
  const stringValue = value.toFixed(6);
...
export function convertToMetricPrefix(numberStr: string) {
  ...
  return resultStr + ' ' + prefixes[count];
}

export function gasPercentage(gasUsed: string, gasAttached: string) {
  if (!gasAttached) return 'N/A';

  const gasUsedBig = new Big(gasUsed);
  const gasAttachedBig = new Big(gasAttached);

  const percentage = gasUsedBig.div(gasAttachedBig).times(100).toFixed(2);
