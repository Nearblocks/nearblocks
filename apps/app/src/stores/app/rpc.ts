...
export const useRpcStore = create<RpcState>((set, get) => ({
  ...
  rpc: '',
  initializeRpc: (serverRpcCookie: string, defaultProviders: RpcProvider[]) => {
    const currentRpc = serverRpcCookie;
    if (!currentRpc && defaultProviders.length > 0) {
      ...
      set({ rpc: currentRpc, providers: defaultProviders });
    }
  },
