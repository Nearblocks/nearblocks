...
import sql from '#sql/intents';

const txns = responseHandler(
  response.txns,
  async (req: RequestValidator<IntentsTxnsReq>) => {
    ...
    const eventsQuery: WindowListQuery<
      Omit<IntentsTxn, 'block' | 'transaction_hash'>
    > = (start, end, limit) => {
    ...
    const txns = await dbBase.manyOrNone<IntentsTxn>(unionQuery);
...
const txnCount = responseHandler(
  response.count,
  async (req: RequestValidator<IntentsTxnCountReq>) => {
    const before = req.validator.before_ts;
