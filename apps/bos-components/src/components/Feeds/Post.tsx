...
export default function (props: Props) {
  ...
  const data: any = Social.get(`${accountId}/post/main`, blockHeight);
  ...
  const [onHashtag] = useState(() => (hashtag: any) => (
    <span key={hashtag} className="d-inline-flex text-nearblue-700">
  ...
  return (
    <>
      ...
      {state.showReply && <div className="border-top"></div>}
      {props.customComments
        ...
        : !props.hideComments && (
            <Widget
              key="comments"
              loading={false}
              src={`${props.ownerId}/widget/bos-components.components.Feeds.CommentFeed`}
              props={{
                item,
