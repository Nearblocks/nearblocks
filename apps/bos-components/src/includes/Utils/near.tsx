...
export default function () {
  ...
  function decodeArgs(args: string[]) {
    if (!args || typeof args === 'undefined') return {};

    return JSON.parse(Buffer.from(args, 'base64').toString());
  }

  function tokenAmount(amount: string, decimal: string, format: boolean) {
  ...
  function price(amount: string, decimal: string, price: string) {
  ...
  function mapRpcActionToAction(action: string | ActionType) {
  ...
  function txnActions(txn: RPCTransactionInfo) {
    const txActions = [];
  ...
  function txnErrorMessage(txn: RPCTransactionInfo) {
  ...
  function mapRpcInvalidAccessKeyError(error: RPCInvalidAccessKeyError) {
  ...
  function mapRpcCompilationError(error: RPCCompilationError) {
    const UNKNOWN_ERROR = { type: 'unknown' };
  ...
  function mapRpcFunctionCallError(error: RPCFunctionCallError) {
    const UNKNOWN_ERROR = { type: 'unknown' };
  ...
  function mapRpcReceiptActionError(error: ActionError) {
    const UNKNOWN_ERROR = { type: 'unknown' };
    const { kind } = error;
  ...
  function mapRpcReceiptInvalidTxError(error: InvalidTxError) {
    const UNKNOWN_ERROR = { type: 'unknown' };
  ...
  function mapRpcReceiptError(error: TxExecutionError) {
  ...
  function mapRpcReceiptStatus(status: ExecutionStatusView) {
  ...
  function mapRpcActionToAction1(rpcAction: NonDelegateActionView) {
  ...
  function parseOutcomeOld(outcome: ParseOutcomeInfo) {
    return {
