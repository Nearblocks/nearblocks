...
export const poolIdsCheck = async () => {
  ...
  await dbBase('validator_data').update('pool_ids', JSON.stringify(accounts));
};
...
const fetchStakingPoolInfo = async () => {
  const data = await dbBase('validator_data')
    .select('mapped_validators')
    .first();
...
export const updateStakingPoolStake = async () => {
  const data = await dbBase('validator_data').select('stake_proposals').first();
...
const saveValidatorLists = async () => {
  const data = await dbBase('validator_data').select('*').first();
...
export const validatorsCheck = async () => {
  ...
  if (validators) {
    const datas = await dbBase('validator_data').select('*').first();
...
export const validatorsTelemetryCheck = async () => {
  ...
  if (validators) {
    ...
    const nodesInfo = await dbBase('nodes')
      .select(
        'ip_address',
        'account_id',
        'node_id',
