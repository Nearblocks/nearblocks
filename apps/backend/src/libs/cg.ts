...
const marketChart = async (
  ...
  try {
    // Demo api
    // const resp = await axios.get<CGMarketChart>(
    //   `https://api.coingecko.com/api/v3/coins/${id}/market_chart/range?vs_currency=usd&from=1767024000&to=1777564800`,
    //   { headers: { 'x-cg-demo-api-key': config.cgApiKey }, timeout: 60000 },
    // );
    const resp = await axios.get<CGMarketChart>(
      `https://pro-api.coingecko.com/api/v3/coins/${id}/market_chart/range?vs_currency=usd&from=${from}&to=${to}`,
      { headers: { 'x-cg-pro-api-key': config.cgApiKey }, timeout: 60000 },
    );

    return resp.data?.prices ?? null;
  } catch (error) {
    logger.error(`tokenPrices: cg.marketChart: ${id}`);
    logger.error(error);
    return null;
  }
};
