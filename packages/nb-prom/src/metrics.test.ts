...
describe('createSyncMetrics', () => {
  ...
  it('returns object with expected keys', () => {
    ...
    expect(sync).toHaveProperty('lastBlockTimestamp');
  });
  ...
  it('lastBlockTimestamp is registered and reports value', async () => {
    const sync = createSyncMetrics(register);
    sync.lastBlockTimestamp.set(1711900000);
    const metrics = await register.getMetricsAsJSON();
    const found = metrics.some(
      (m: Metric) => m.name === 'indexer_last_block_timestamp_seconds',
    );
    expect(found).toBe(true);
  });
});
